...
*/
class ArmSegment extends Model
{
   /**
      @param name   {@link String} name for this segment of a robot arm
      @param color  {@link Color} for this segment of a robot arm
   */
   constructor(name, color)
   {
      ...
      ModelShading.setColor(this, color);
   }
}
...
// First arm.
arm1_s.setModel(new ArmSegment("Arm-1, shoulder", Color.blue));
...
arm1_e1.setModel( new ArmSegment("Arm-1, elbow-1", Color.blue) );
arm1_e2.setModel( new ArmSegment("Arm-1, elbow-2", Color.blue) );
...
arm1_w1.setModel( new ArmSegment("Arm-1, wrist-1", Color.blue) );
arm1_w2.setModel( new ArmSegment("Arm-1, wrist-2", Color.blue) );
...
arm1_f1.setModel( new ArmSegment("Arm-1, finger-1", Color.blue) );
arm1_f2.setModel( new ArmSegment("Arm-1, finger-2", Color.blue) );

// Second arm.
arm2_s.setModel(new ArmSegment("Arm-2, shoulder", Color.red));
...
arm2_e1.setModel( new ArmSegment("Arm-2, elbow-1", Color.red) );
arm2_e2.setModel( new ArmSegment("Arm-2, elbow-2", Color.red) );
...
arm2_w1.setModel( new ArmSegment("Arm-2, wrist-1", Color.red) );
arm2_w2.setModel( new ArmSegment("Arm-2, wrist-2", Color.red) );
...
arm2_f1.setModel( new ArmSegment("Arm-2, finger-1", Color.red) );
arm2_f2.setModel( new ArmSegment("Arm-2, finger-2", Color.red) );
...
function runOffline()
{
   ...
   for(let rot = -45; rot <= 45; rot += 5)
   {
      arm_p[0].matrix2Identity()
               ...
               .mult(Matrix.scaleXYZ(shoulderLength[0],
                                     shoulderLength[0],
                                     1));
   ...
   console.log("Wall Clock Time: " + (endTime - startTime));
}