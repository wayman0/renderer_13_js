...
// Create the shoulder model
const shoulder = Model.buildName("shoulder_segment");
...
// Add the shoulder Model to the shoulder's Position.
shoulder_p.setModel(shoulder);

// Push the position away from where the camera is.
shoulder_p.setMatrix( Matrix.translate(0, 0, -1) );

// Create the elbow model
const elbow = Model.buildName("elbow_segment");
...
// add the elbow model to the elbow position
elbow_p.setModel(elbow);

// Add the elbow position to the shoulder's position
shoulder_p.addNestedPosition(elbow_p);

// Position the elbow
elbow_p.setMatrix(Matrix.translate(shoulderLength, 0, 0));

// Create the wrist model
const wrist = Model.buildName("wrist_segment");
...
// Add the wrist model to the wrist position
wrist_p.setModel(wrist);

// add the wrist position to the elbow position
elbow_p.addNestedPosition(wrist_p);

// position the wrist 
wrist_p.setMatrix(Matrix.translate(elbowLength, 0, 0));

// create the finger model
const finger = Model.buildName("finger_segment");
...
// add the finger model to the finger position
finger_p.setModel(finger); 

// add the finger position to the wrist position
wrist_p.addNestedPosition(finger_p);

// position the finger
finger_p.setMatrix(Matrix.translate(wristLength, 0, 0));
...
function setTransformations()
{
   ...
   elbow_p.matrix2Identity()
            .mult(Matrix.translate(shoulderLength, 0, 0))
            .mult(Matrix.rotateZ(elbowRotation));

   wrist_p.matrix2Identity()
            .mult(Matrix.translate(elbowLength, 0, 0))
            .mult(Matrix.rotateZ(wristRotation));

   finger_p.matrix2Identity()
      .mult(Matrix.translate(wristLength, 0, 0))
