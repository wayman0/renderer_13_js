...
*/
//@ts-check
import {Model, Vertex, LineSegment, LineFan, LineLoop} from "../scene/SceneExport.js";
...
export default class TriangularPyramid extends Model
{
   ...
   */
   ...
   {
      super(undefined, undefined, undefined, undefined, undefined,  format("Triangular_Pyramid(%.2f,%.2f,%d,%d)", r,   h,   n, k));
      ...
      const botFan = new LineFan();
      const topFan = new LineFan();

      botFan.addIndex(centerIndex);
      topFan.addIndex(apexIndex);
      ...
      for(let j = 0; j < k; ++j)
      {
         ...
         botFan.addIndex(index+0, index+1, index+2);
         topFan.addIndex(index+0, index+1, index+2);
         index += 3;
      }

      this.addPrimitive(botFan, topFan);
      ...
      for(let i = 0; i < n; ++i)
      {
         ...
         this.addPrimitive(new LineLoop(index+0, index+1, index+2));
