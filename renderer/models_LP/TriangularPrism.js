...
import {Model, Vertex, LineSegment, LineFan, LineLoop, Lines} from "../scene/SceneExport.js";
...
//
   ...
   */
   constructor(r=.5, h=.6, h2=r*Math.tan(Math.atan(Math.sqrt(2))), n=3, bothHalves=true)
   {
      ...
      // 3 top faces
      this.addPrimitive(new LineFan(6, 0, 1, 2));
      // the top edge
      this.addPrimitive(new LineLoop(0, 1, 2));
                                                              // three vertical edges
      this.addPrimitive(new Lines(0,3, 1,4, 2,5));

      // the bottom edge
      this.addPrimitive(new LineLoop(3, 4, 5));

      // 3 bottom faces
      this.addPrimitive(new LineFan(7, 3, 4, 5));
