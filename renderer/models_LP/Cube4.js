...
import { LineFan, LineLoop, LineStrip, Lines } from "../scene/SceneExport";
...
export default class Cube4 extends Model
{
   ...
   */
   constructor(xCount=2, yGrid=1, zCount=2)
   {
      ...
      const bottomFan = new LineFan();
      const topFan    = new LineFan();

      bottomFan.addIndex(cBottom);
      topFan.addIndex(cTop);
      for(let j = 0; j < 2*xCount + 2*zCount; ++j)
      {
         bottomFan.addIndex(j);
         topFan.addIndex((yGrid+1) * (2*xCount+2*zCount) + j);
      }
      this.addPrimitive(bottomFan, topFan);
      ...
      for (let i = 0; i < 2 + yGrid; ++i) // choose a height of latitude
      {
         ...
         for(let j = 0; j < 2*xCount + 2*zCount; ++j)
            lineLoop.addIndex(i*(2*xCount + 2*zCount) + j);

         this.addPrimitive(lineLoop);
      }
      ...
      for (let j = 0; j < 2*xCount + 2*zCount; ++j)
      {
         ...
         for(let i = 0; i < 2+yGrid; ++i)
            lineStrip.addIndex(i*(2*xCount+2*zCount)+j);
