 ...
 * @param {Vertex} vert vertex in the cameras view coordinate system
 * @returns {Vertex} a vertex in the normalized camera coordinate system
 */
export default function view2camera(vert)
{
   ...
   {
       v_z = vert.z;
       v_x = vert.x - v_z * (r + l)/(2 * near);
       v_y = vert.y - v_z * (t + b)/(2 * near);
       v_x = (2 * near * v_x)/(r - l);
       v_y = (2 * near * v_y)/(t - b);
   }
   else
