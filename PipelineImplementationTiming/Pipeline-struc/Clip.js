...
{
    logPrimitive("5. Clipping", this.model, prim);

    let pClipped = undefined;
    
    if (prim instanceof LineSegment)
        pClipped = LineClip(this.model, /**@type {LineSegment}*/ (prim));
    else if(prim instanceof Point)
        pClipped = PointClip(this.model, /**@type {Point}*/ (prim));

    if (pClipped != undefined)
    {
        logPrimitive("5. Clipped (accept)", this.model, pClipped);
        return pClipped;
    }
    else
        logPrimitive("5. Clipped (reject)", this.model, prim);
}
