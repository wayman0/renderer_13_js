...
export function clip(prim)
{
    ...
    logPrimitive("3. Near_Clipping", this.model, prim);

    let pClipped = undefined;

    if (prim instanceof LineSegment)
        pClipped = NearLine( this.model, /**@type {LineSegment}*/(prim), this.camera);
    else if(prim instanceof Point)
        pClipped = NearPoint(this.model, /**@type {Point}*/(prim),       this.camera);

    if (pClipped != undefined)
    {
        logPrimitive("3. Near_Clipped (accept)", this.model, pClipped);
        return pClipped;
    }
    else
        logPrimitive("3. Near_Clipped (reject)", this.model, prim);
}
