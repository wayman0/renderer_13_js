...
//@ts-check
import {Model, Vertex, Camera, Position} from "../../renderer/scene/SceneExport.js";
 ...
 * @param {Vertex} vert the vertex to be projected
 * @returns {Vertex} the projected vertex
 */
export function projectModel(vert)
{
   if(cam.perspective)
...
export function projectNestedModel(model, camera)
{
   ...
   cam = camera;
   const mod2 = new Model( model.vertexList.map(projectModel),
                           model.primitiveList, 
                           model.colorList, 
                           model.matrix, 
                           model.nestedModels, 
