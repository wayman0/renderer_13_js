...
import {Camera, Position, Model, Vertex} from "../../renderer/scene/SceneExport.js";
...
/**
   ...
   @param {Vertex} vert   {@link Vertex} object in the world coordinate system
   @return {Vertex} a new {@link Vertex} object in the {@link Camera}'s view coordinate system
*/
export function world2viewModel(vert)
{
    // We translate each vertex in the opposite
    // direction of what the camera was translated by
    const negViewVector = cam.getViewVector().timesScalar(-1);
...
export function world2viewNestedModel(model, camera)
{
   ...
   cam = camera;
   const mod2 = new Model( model.vertexList.map(world2viewModel),
                           model.primitiveList, 
                           model.colorList, 
                           model.matrix, 
                           model.nestedModels, 
